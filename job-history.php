...
<head>
    ...
    $objModule->redirect("./job-history.php");
...
<div class="mid">
    <div class="wrapper">
        ...
        <div class="content_part">
            ...
            <!----Sidebar end---->
            <div class="blog_part">
                <h2><span>Job History</span></h2>
                <?php
                $arrJobHistory=$objData->getAll("select p.id,p.uid,p.title,p.description,b.Bid_amt,p.win_date,p.end_date from tbl_post p left join tbl_bidding b on p.id=b.Post_id where p.win_status='4' and b.Uid=".$_SESSION['clg_userid']);
                ?>
                                ...
                                if(count($arrJobHistory)>0)
                                {
                                    foreach($arrJobHistory as $history)
                                    {
                                        ...
                                        <li>
                                            <ul>
                                                <li>
                                                    <h4><a href="<?php echo $objModule->SITEURL;?>detail-bidded.php?post_id=<?php echo $history['id'];?>&job=act&post_uid=<?php echo $history['uid'];?>"><?php echo $history['title']; ?></a></h4>
<?php echo date('M Y',strtotime($history['win_date'])); ?> - <?php echo date('M Y',strtotime($history['end_date'])); ?><br />
                                                    ...
                                                    <?php if($arrCount[0]['tcnt']==0):?>
                                                        <a data-post="<?php echo $history['id'];?>" data-to="<?php echo $history['uid'];?>" class="various1" href="#inline_content">Add Review</a>
                                                    <?php endif;?> 
                                                </li>
                                                <?php
                            ...
                            <?php
                                ...
                                {
        echo "<h3>No Job completed Yet!!! <a href='search_jobs.php?explor=".base64_encode(rand())."'>Click here</a> to crack one</h3>";
                                }
                                ?>
            </div>
