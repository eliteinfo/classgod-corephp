...
$strGroupby =   "tp.id";
$arrPostT = $objModule->getAll($strTable,array("tp.`id` AS post_id","tp.title","tm.`From_user`","tm.`To_user`"),$strCond,$strGroupby,"tp.id DESC",$intStart,$intNum);
  ...
  <div class="blog_part">
  	<h2><span>Messages</span></h2>
        <ul class="jobmsg-list">
            <?php if(!empty($arrPostT)): ?>
                <?php foreach($arrPostT as $intK1=>$strV1):
                    $arrTempUnread = $objModule->getAll("SELECT COUNT(*) as tunred FROM `tbl_messages` tm WHERE tm.`To_user` = '".$_SESSION['clg_userid']."' AND tm.Post_id = '".$strV1['post_id']."'  AND tm.`is_read` = '0'");
                ?>
            <li><div class="post-dt"><?php echo $arrTempUnread[0]['tunred'];?> msg</div><div class="jobtitle"><h2><a href="message_post.php?post_id=<?php echo $strV1['post_id'];?>"><?php echo $strV1['title'];?></a></h2></div></li>
                <?php endforeach;?>
            <?php endif;?>
        </ul>
        <!----navigation Start---->
